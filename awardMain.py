...
for i in range (1, n_row + 1):
    # Open the raw Image
    img = Image.open('CertificateTemplates/blankCertificate_v2.png')
    ...
    scoreAward = worksheet.cell(row = i+1, column = 1)   # column 1, Award
    ...
    # if there is an empty cell for the score, continue to the next row
    if scoreAward_value is None:
        continue
    
    # Custom font style and font size
    arialBoldFont = ImageFont.truetype("Fonts/arial-bold.ttf", 50)
    ...
    draw.text((W/2,H/2-110), scoreAward_value, font=arialBoldFont, fill=(0, 0, 0), anchor="mm")
    ...
    # add image file paths to column "H"
    filePath = worksheet.cell(row = i+1, column = 8)   # "H" is column #8
